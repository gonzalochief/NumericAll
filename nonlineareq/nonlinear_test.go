...
func TestBisect(t *testing.T) {
	// Set test cases
	testCases := make([]testStructureBisect, 3)
	...
	for _, tc := range testCases {
		c, _, _, _ := BisectBolzano(tc.TestFunction, tc.TestA, tc.TestB, tc.TestTol)
...
func TestRegulaFalsi(t *testing.T) {
	testCases := make([]testStructRegulaFalsi, 1)
	...
	for _, tc := range testCases {
		c, yC, absErr, _ := RegulaFalsi(tc.TestFunction, tc.TestA, tc.TestB, tc.TestTol, tc.TestEpsilon, 50)
